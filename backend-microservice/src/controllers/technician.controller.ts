...
export const getTechnicianDashboard = async (req: Request, res: Response) => {
    try {
        // Count repairs
        const repairsCount = await TechnicalRepair.countDocuments({}) || 0;
        ...
        const repairQueue = await TechnicalRepair.find()
            .sort({ createdAt: -1 })
            .limit(5)
            .lean();
        ...
        const schedule = await Meeting.find({
        ...
        const stats = {
            ...
            avg_time: '2.4h',
            first_fix: '96%',
            parts_requested: inventoryStats[0]?.lowStock || 14,
            rating: '4.9',
            repairs_trend: 12,
            time_trend: -8,
            fix_trend: 3
        ...
        const repair_queue = repairQueue.length > 0 ? repairQueue.map((r: any) => ({
            id: r._id,
        ...
        const scheduleData = schedule.length > 0 ? schedule.map((s: any, i: number) => ({
        ...
        const chart_data = {
            ...
            parts_status: {
                in_stock: inventoryStats[0]?.total - inventoryStats[0]?.lowStock || 20,
                low_stock: inventoryStats[0]?.lowStock - inventoryStats[0]?.outOfStock || 8,
                out_of_stock: inventoryStats[0]?.outOfStock || 3
            },
            performance: [
                { label: 'Mon', value: 4 },
                { label: 'Tue', value: 6 },
...
export const getTechnicianJobs = async (req: Request, res: Response) => {
    try {
        const jobs = await TechnicalRepair.find()
            .sort({ createdAt: -1 })
            .limit(20)
            .lean();
...
export const getTechnicianWorkOrders = async (req: Request, res: Response) => {
    try {
        const workOrders = await Task.find({ type: 'work_order' }).sort({ createdAt: -1 }).limit(20).lean();
...
// Inventory endpoint
export const getTechnicianInventory = async (req: Request, res: Response) => {
    try {
        const inventory = await Inventory.find().sort({ quantity: 1 }).limit(30).lean();
...
// Bookings endpoint
export const getTechnicianBookings = async (req: Request, res: Response) => {
    try {
        const bookings = await Meeting.find().sort({ date: 1 }).limit(10).lean();
