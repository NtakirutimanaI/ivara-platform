...
<div class="container">
    ...
    <div class="card card-left">
        ...
        <form action="{{ url('/admin/meetings/create') }}" method="POST">
            @csrf
            <label>Time</label>
            ...
            <div class="link-buttons">
                <button type="button" class="bg-teal" onclick="generateGoogleMeet()">Meet</button>
            ...
            <button type="submit" class="bg-blue">Create</button>
        </form>
    </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $meeting->id }}</td>
                    <td>{{ $meeting->time }}</td>
                    <td><a href="{{ $meeting->link }}" target="_blank" class="text-blue-600">Open</a></td>
                    <td>{{ $meeting->description }}</td>
                    <td>{{ $meeting->status == 'Unpublished' ? 'No role' : (is_array($roles) ? implode(', ', $roles) : '') }}</td>
                    <td class="flex">
                        ...
                        <form action="{{ url('/admin/meetings/'.$meeting->id.'/delete') }}" method="POST" onsubmit="return confirm('Are you sure?')">
                            @csrf
                            <button type="submit" class="bg-red">Del</button>
                        </form>
                        @if($meeting->status == 'Unpublished')
                            <button onclick="openPublishModal({{ $meeting->id }})" class="bg-blue">Pub</button>
                        @else
                            <form action="{{ url('/admin/meetings/'.$meeting->id.'/unpublish') }}" method="POST">
                                @csrf
...
<div id="editModal" class="modal">
    <div class="modal-content">
        ...
        <form id="editForm" method="POST">@csrf
            ...
            <label>Link</label>
            <input type="url" name="link" id="editLink" required>
            <label>Description</label>
    ...
    <div class="modal-content">
        ...
        <form id="publishForm" method="POST">@csrf
            <select name="roles[]" id="publishRoles" multiple required>
                ...
                <option value="craftsperson">Tailor</option>
...
function openViewModal(id){
    ...
    const roles = meeting.status === 'Unpublished' ? 'No role' : (meeting.roles ? meeting.roles.join(', ') : '');
...
function openEditModal(id){
    ...
    document.getElementById('editForm').action = `/admin/meetings/${id}/update`;
    document.getElementById('editModal').classList.add('flex');
}
function closeEditModal(){ document.getElementById('editModal').classList.remove('flex'); }

function openPublishModal(id){
    document.getElementById('publishForm').action = `/admin/meetings/${id}/publish`;
