...
<div class="container">
  <h1>Manager Product Management</h1>
  ...
  <div class="product-form">
    <form action="{{ route('manager.create_products.store') }}" method="POST" enctype="multipart/form-data">
      @csrf
      ...
      <input type="number" name="price" step="0.01" id="price" placeholder="Enter price in FRW" required>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>{{ number_format($product->price, 2) }} FRW</td>
        ...
        <td class="actions">
          ...
          <form action="{{ route('manager.create_products.destroy', $product->id) }}" method="POST" style="display:inline;">
            @csrf
            @method('DELETE')
          ...
          <form action="{{ route('manager.create_products.publish', $product->id) }}" method="POST" style="display:inline;">
            @csrf
            <button class="publish">Publish</button>
          </form>
          @else
          <form action="{{ route('manager.create_products.unpublish', $product->id) }}" method="POST" style="display:inline;">
            @csrf
...
<script>
  function openEditModal(id, name, description, price) {
    ...
    document.getElementById('editForm').action = '/manager/create-products/' + id;
