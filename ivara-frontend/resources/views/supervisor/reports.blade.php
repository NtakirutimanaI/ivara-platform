...
<style>
.reports-container {width:80%; margin-left:240px; margin-top:80px; padding:20px; font-family:Arial,sans-serif;}
...
th {background:#4f46e5; color:#fff; position:sticky; top:0; z-index:2;}
...
#exportPdfBtn {background:#4f46e5;}
...
<div class="reports-container">
    ...
    {{-- KPI CARDS --}}
    <div class="kpi-cards">
        <div class="card"><h2 id="kpiRevenue">${{ number_format($totalRevenue,2) }}</h2><p>Total Revenue</p></div>
    ...
    <div class="kpi-cards">
        <div class="card"><h2 id="kpiProfits">${{ number_format($totalProfits,2) }}</h2><p>Total Profits</p></div>
        <div class="card"><h2 id="kpiLosses">${{ number_format($totalLosses,2) }}</h2><p>Total Losses</p></div>
    ...
    <div class="section" data-type="subscriptions">
        ...
        <table>
            ...
            <tbody>
                @foreach($subscriptions as $sub)
                <tr><td>{{ $sub->id }}</td><td>{{ $sub->user->name ?? 'N/A' }}</td><td>{{ $sub->status }}</td><td>${{ $sub->price ?? 0 }}</td></tr>
                @endforeach
            </tbody>
        </table>
    </div>
    ...
    <div class="chart-container">
        ...
        <p id="chartSummary">
            <strong>Devices:</strong> Revenue ${{ number_format($deviceRevenue,2) }}, Profit ${{ number_format($deviceProfit,2) }}, Cost ${{ number_format($devicesCost,2) }}<br>
            <strong>Subscriptions:</strong> Revenue ${{ number_format($subscriptionsRevenue,2) }}, Profit ${{ number_format($subscriptionProfit,2) }}, Cost ${{ number_format($subscriptionsCost,2) }}<br>
            <strong>Inventory:</strong> Revenue ${{ number_format($inventoryRevenue,2) }}, Profit ${{ number_format($inventoryProfit,2) }}, Cost ${{ number_format($inventoryCost,2) }}
        </p>
...
document.getElementById('toggleStacked').addEventListener('click', function(){
    ...
    chartSummary.innerHTML = `<strong>Devices:</strong> Revenue ${{ number_format($deviceRevenue,2) }}, Profit ${{ number_format($deviceProfit,2) }}, Cost ${{ number_format($devicesCost,2) }}<br>
        <strong>Subscriptions:</strong> Revenue ${{ number_format($subscriptionsRevenue,2) }}, Profit ${{ number_format($subscriptionProfit,2) }}, Cost ${{ number_format($subscriptionsCost,2) }}<br>
        <strong>Inventory:</strong> Revenue ${{ number_format($inventoryRevenue,2) }}, Profit ${{ number_format($inventoryProfit,2) }}, Cost ${{ number_format($inventoryCost,2) }}`;
});
...
// --- PDF Export with page numbers, repeated headers, polished colors ---
document.getElementById('exportPdfBtn').addEventListener('click', async function(){
    ...
    // Clone container to hide buttons
