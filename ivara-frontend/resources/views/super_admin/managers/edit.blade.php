...
<div class="dashboard-wrapper">
    ...
    <header class="pro-header">
        ...
        <div>
            <a href="{{ route('super_admin.managers.index') }}" class="btn-glass secondary">
    ...
    <div class="pro-card glass-panel" style="max-width: 800px; margin: 0 auto;">
        <form id="editManagerForm" onsubmit="handleUpdateManager(event)">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 25px;">
                ...
                <div>
                    ...
                    <select class="glass-input" name="role">
                        <option value="Area Manager" {{ $admin['role'] == 'Area Manager' ? 'selected' : '' }}>Area Manager</option>
                        <option value="Category Manager" {{ $admin['role'] == 'Category Manager' ? 'selected' : '' }}>Category Manager</option>
                        <option value="Operations Manager" {{ $admin['role'] == 'Operations Manager' ? 'selected' : '' }}>Operations Manager</option>
                    </select>
                </div>
            ...
            <div style="margin-top: 35px; display: flex; justify-content: flex-end; gap: 10px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
                <a href="{{ route('super_admin.managers.index') }}" class="btn-glass secondary" style="background: transparent; border: 1px solid #e5e7eb;">Cancel</a>
...
<script>
    function handleUpdateManager(e) {
        e.preventDefault();
        const form = document.getElementById('editManagerForm');
        const formData = new FormData(form);
        formData.append('_method', 'PUT');

        fetch("{{ route('super_admin.managers.update', $id) }}", {
            method: "POST", 
            headers: {
        ...
        .then(data => {
            if(data.success) {
                ...
                setTimeout(() => {
                    window.location.href = "{{ route('super_admin.managers.index') }}";
                }, 1000);
            } else {
                toastr.error('Error updating manager');
            }
        })
        .catch(error => {
