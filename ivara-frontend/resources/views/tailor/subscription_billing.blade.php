...
<div class="container-fluid tailorperson-container">
                        ...
                        <div class="modal fade" id="upgradeModal{{ $sub['id'] }}" tabindex="-1" aria-labelledby="upgradeModalLabel{{ $sub['id'] }}" aria-hidden="true">
                          <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                              ...
                              <form action="{{ route('tailor.subscription.pay', $sub['id']) }}" method="POST">
                                  @csrf
                                  <div class="modal-body">
                                          ...
                                          <select name="plan" class="form-select" required onchange="updatePriceModal(this, {{ $sub['id'] }})">
                                              ...
                                              @foreach($availablePlans as $plan)
                                                  <option value="{{ $plan['name'] }}" data-price="{{ $plan['price'] }}" {{ $sub['plan'] === $plan['name'] ? 'selected' : '' }}>
                                                      {{ $plan['name'] }} ({{ number_format($plan['price']) }} FRW)
                                                  </option>
                                              @endforeach
                                          </select>
                                      </div>
                                      ...
                                      <p class="text-muted">Selected Plan Price: <span id="modal-price-display-{{ $sub['id'] }}">{{ number_format($sub['price']) }}</span> FRW</p>
                                  </div>
...
@media (max-width: 1200px) {
    .tailorperson-container { width: 95%; margin-left: auto; }
}
@media (max-width: 768px) {
    .tailorperson-container { margin-left: auto; margin-top: 60px; }
